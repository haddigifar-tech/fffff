...
const PublicPackages: React.FC<PublicPackagesProps> = ({ packages, addOns, userProfile, showNotification, setClients, setProjects, setTransactions, setCards, setLeads, addNotification, cards, promoCodes, setPromoCodes }) => {
    const { createRecord, updateRecord } = useSupabaseData();
    ...
    const handleSubmit = async (e: React.FormEvent) => {
        ...
        try {
            await createRecord('clients', newClient);
            await createRecord('projects', newProject);
            await createRecord('leads', newLead);

            // Update local state after successful Supabase write
            setClients(prev => [newClient, ...prev]);
            setProjects(prev => [newProject, ...prev]);
            setLeads(prev => [newLead, ...prev]);

            if (promoCodeAppliedId) {
                const promoToUpdate = promoCodes.find(p => p.id === promoCodeAppliedId);
                if (promoToUpdate) {
                    await updateRecord('promo_codes', promoToUpdate.id, { usageCount: promoToUpdate.usageCount + 1 });
            ...
            if (dpAmount > 0) {
                ...
                await createRecord('transactions', newTransaction);
                setTransactions(prev => [...prev, newTransaction].sort((a,b) => new Date(b.date).getTime() - new Date(a.date).getTime()));

                const cardToUpdate = cards.find(c => c.id === destinationCard.id);
                if (cardToUpdate) {
                    await updateRecord('cards', cardToUpdate.id, { balance: cardToUpdate.balance + dpAmount });

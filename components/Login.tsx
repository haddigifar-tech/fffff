...
// using signIn/signUp handlers passed from parent (useSupabaseData)
...
const Login: React.FC<LoginProps> = ({ onSignIn, onSignUp }) => {
    ...
    const handleSubmit = async (e: React.FormEvent) => {
        ...
        try {
            ...
            // onSignIn will update auth state in parent; no further action here
        } catch (err: any) {
                 ...
                 <div className="bg-white p-8 rounded-2xl shadow-lg border border-slate-200">
                    ...
                    <button className="w-full h-12 px-4 bg-white border border-slate-300 rounded-lg flex items-center justify-center gap-2 text-slate-700 font-semibold hover:bg-slate-50 transition-colors">
                     ...
                     <div className="text-center mt-6 text-xs text-slate-400 bg-slate-50 p-3 rounded-lg">
                        <p className="font-bold text-slate-500">Akun Login (Password Baru):</p>
                        <p><span className="font-semibold text-slate-600">admin@venapictures.com</span> (pw: admin123)</p>
                        <p><span className="font-semibold text-slate-600">sarah@venapictures.com</span> (pw: sarah123)</p>
                        <p><span className="font-semibold text-slate-600">rio@venapictures.com</span> (pw: rio123)</p>
                        <p><span className="font-semibold text-slate-600">maya.freelancer@email.com</span> (pw: maya123)</p>
                    </div>
